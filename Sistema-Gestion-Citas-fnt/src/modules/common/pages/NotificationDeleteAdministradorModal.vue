...
const props = defineProps<{ isVisible: boolean, administrador: Administrador }>();
...
const confirm = () => {
    console.log(props.administrador.id_administrador);
    deleteAdministrador();
};
...
const deleteAdministrador = async () => {
    ...
    const apiUrl = `http://localhost:8080/Administrador/EliminarAdministrador/${props.administrador.id_administrador}`;

    try {
        await axios.delete(apiUrl);
        notificationMessage.value = 'Administrador eliminado con éxito';
        notificationType.value = 'success';
        // Aquí puedes agregar lógica adicional para actualizar la lista de pacientes después de eliminar.
    } catch (error: any) {
        notificationMessage.value = 'Error al eliminar al administrador';
