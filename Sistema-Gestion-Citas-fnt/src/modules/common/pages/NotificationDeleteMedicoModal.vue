...
import type { Medico } from './InterfaceMedico';
import axios from 'axios';
import loading from 'vue-loading-overlay';
import 'vue-loading-overlay/dist/css/index.css';

// Props para controlar la visibilidad del modal desde el componente padre
const props = defineProps<{ isVisible: boolean, medico: Medico }>();
...
const confirm = () => {
   // console.log(props.medico.id_medico);
    deleteMedico();
};
...
const deleteMedico = async () => {
    ...
    const apiUrl = `http://localhost:8080/Medico/EliminarMedico/${props.medico.id_medico}`;

    try {
        await axios.delete(apiUrl);
        notificationMessage.value = 'Medico eliminado con éxito';
        notificationType.value = 'success';
        // Aquí puedes agregar lógica adicional para actualizar la lista de pacientes después de eliminar.
    } catch (error: any) {
        notificationMessage.value = 'Error al eliminar al médico, tiene una cita pendiente';
