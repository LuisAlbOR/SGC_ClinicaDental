...
const props = defineProps<{ isVisible: boolean, cita: Cita }>();
...
const confirm = () => {
    console.log(props.cita.id_cita);
    deleteCita();
};
...
const deleteCita = async () => {
    ...
    const apiUrl = `http://localhost:8080/Cita/EliminarCita/${props.cita.id_cita}`;

    try {
        await axios.delete(apiUrl);
        notificationMessage.value = 'Cita eliminada con éxito';
        notificationType.value = 'success';
        // Aquí puedes agregar lógica adicional para actualizar la lista de pacientes después de eliminar.
    } catch (error: any) {
        notificationMessage.value = 'Error al eliminar la cita';
